...
(function () {
  ...
  const CHAR_SELECT_ORDER = [
    ...
    "killua", "kurapika", "hisoka", "neferpitou", "kuroko", "gunha", "shokuhou", "rakko", "accelerator", "saber", "gilgamesh",
    ...
    "megumin", "aqua", "kaiba", "pegasus", "jaden", "jesse", "yubel", "toudou", "adam", "gasai", "minene",
  ...
  const ASSET_VERSION = "2026-09-12";
  ...
  S.net.on("nexus_state", (m) => {
    ...
    // A "GUEST" universe (a show not in the Nexus yet) or a BLANK one (a disk-only bucket with no
    // nexus_meta row) both funnel into the catch-all "Guest" tab so nothing is left ungrouped.
    (m.sets || []).forEach((s) => { buckets[s[0]] = s[1]; if (s[2] != null) bucketUni[s[0]] = prettyUniverse(String(s[2])) || "Guest"; });
  ...
  function nexusMenu() {
    ...
    const universes = Object.values(byKey).sort((a, b) => {
      const ag = uniKey(a) === "guest", bg = uniKey(b) === "guest";   // the Guest catch-all sorts last
      return ag !== bg ? (ag ? 1 : -1) : a.localeCompare(b);
    });
